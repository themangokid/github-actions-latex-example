...
COMPILE_DIR = os.path.join(PROJ, "compile")

def write_book_wrappers():
    """Generate compile/<CODE>.tex — one standalone file per book."""
    os.makedirs(COMPILE_DIR, exist_ok=True)
    for code, _, testament, render_base in BOOKS:
        render_dir = f"render/{'GT' if testament == 'GT' else 'NT'}"
        content = (
            "% Auto-generated by usx_to_tex.py — DO NOT EDIT\n"
            "\\input{preamble}\n"
            "\\begin{document}\n"
            f"\\input{{{render_dir}/{render_base}}}\n"
            "\\end{document}\n"
        )
        with open(os.path.join(COMPILE_DIR, f"{code}.tex"), "w", encoding="utf-8") as f:
            f.write(content)
    print(f"  compile/ wrappers written ({len(BOOKS)} files).")
