...
% --- Feature flags (values set in render_config.tex) ---
\newif\ifmobilelayout
\newif\ifwhitebg
\newif\ifshowbackbutton
\newif\ifshowtoc
\input{render_config}
...
\ifwhitebg\pagecolor{white}\else\pagecolor{oldpaper}\fi
\color{textcolor}
...
\newcommand{\ProcessAndRenderFile}[3][]{%
  ...
  \IfStrEq{\firstletter}{J}{%
    \lettrine[loversize=0.21, lines=3, lraise=0.22]{\firstletter}{}\scalebox{0.8}{\MakeUppercase{\firstword}} \remainingtext
  }{%
    \lettrine[loversize=0.3, lines=3, lraise=0.01]{\firstletter}{}\scalebox{0.8}{\MakeUppercase{\firstword}} \remainingtext
  }%
  \par
  \vspace{1cm}
  \ifshowbackbutton\backtotoc\fi
}
...
\ifmobilelayout
  ...
  \renewcommand{\normalsize}{\fontsize{15pt}{18pt}\selectfont}
\else
  \geometry{
    paperwidth=7.7in,
    paperheight=9.5in,
  ...
  \renewcommand{\normalsize}{\fontsize{13pt}{15.2pt}\selectfont}
\fi
